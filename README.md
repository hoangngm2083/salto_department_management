...
| **Công việc (Task)**                 | Toàn quyền mọi transition, mọi project                                | Tạo/gán task **trên project mình là PM**; duyệt (`in_review→done`)/từ chối (`in_review→in_progress`)/huỷ task đó | Tự chuyển trạng thái task **được giao cho mình** (`todo→in_progress→in_review`); xem task được giao, thuộc project mình đang (hoặc từng) là PM, hoặc thuộc project mình có assignment |
| **Bình luận task / Yêu cầu dời hạn** | Toàn quyền                                                            | Bình luận trên mọi task xem được; duyệt/từ chối yêu cầu dời hạn trên project mình là PM                          | Bình luận trên task xem được; tạo/huỷ yêu cầu dời hạn cho task **của chính mình**                                                         |
