...
/**
 * Pending leave requests (and optionally task delay requests), previewed a
 * few at a time with a link into the real list page to act on them - the
 * dashboard just needs to say "this is waiting on you" (mục 9.1), not
 * replicate the full approve/reject UI.
 *
 * Both endpoints already self-scope by actor position on the backend
 * (manager -> own department / employee -> own requests; admin unscoped),
 * so this component doesn't need to know who's viewing it - except for
 * `includeTaskDelayRequests`: unlike leave requests, GetTaskDelayRequestsRequest
 * does NOT scope a manager to the projects they actually manage (mục 7.9/7.10),
 * so a manager here would see requests they have no authority over. Callers
 * pass `includeTaskDelayRequests={false}` for the manager dashboard for that
 * reason; employee (self-scoped) and admin (sees everything, acts on
 * everything) both pass `true`.
 */
export default function DashboardPendingRequests({ title, includeTaskDelayRequests }) {
  ...
  const [leaveRequests, delayRequests] = data ?? [EMPTY_PAGE, EMPTY_PAGE];
  ...
  const items = useMemo(() => {
    ...
    return [...leaves, ...delays];
  }, [leaveRequests, delayRequests]);
